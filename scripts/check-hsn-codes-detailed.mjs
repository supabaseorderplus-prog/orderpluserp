...
async function checkHsnCodesDetailed() {
  console.log('Checking hsn_codes table structure in detail...\n')

  try {
    // Check if company_id column exists by trying to select it
    console.log('Checking for company_id column...')
    ...
    if (testError) {
      console.log('company_id column does not exist or is not accessible:', testError.message)
    } else {
      console.log('✓ company_id column exists!')
      if (testData && testData.length > 0) {
        console.log('Sample with company_id:', JSON.stringify(testData[0], null, 2))
      }
    }
