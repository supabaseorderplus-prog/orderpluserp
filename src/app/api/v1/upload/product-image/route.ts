...
async function ensureBucket() {
  ...
  // Create bucket via direct REST call so service-role JWT is always explicit
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: err instanceof Error ? err.message : 'Failed to upload image' },
      { status: 500 }
    )
  }
}
