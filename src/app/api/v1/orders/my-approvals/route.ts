...
export async function GET(req: NextRequest) {
  try {
    ...
    // Filter for APPROVED orders (check both status and order_status)
    const approvedOrders = (data || []).filter(o => 
      o.status === 'APPROVED' || o.order_status === 'APPROVED'
    )

    console.log(`[My Approvals] Total orders: ${data?.length || 0}, Approved: ${approvedOrders.length}`)
