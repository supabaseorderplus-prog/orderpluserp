...
export async function POST(
  ...
  try {
    ...
    // Get order
      ...
      const dispatchableStatuses = ['PENDING', 'CONFIRMED', 'APPROVED', 'PROCUREMENT', 'IN_PROCUREMENT']
      if (!dispatchableStatuses.includes(order.status)) {
        return NextResponse.json(
          { success: false, message: `Cannot dispatch order with status "${order.status}".` },
          { status: 400 }
        )
      }
    ...
    return NextResponse.json({ success: true, data: updated, message: 'Order dispatched' })
  } catch (err) {
    return NextResponse.json(
      { success: false, message: err instanceof Error ? err.message : 'Dispatch failed' },
      { status: 500 }
    )
  }
}
