...
router.get('/tally', checkPermission('exports', 'view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { from, to } = req.query as { from?: string; to?: string };
    const where: Record<string, unknown> = { status: { in: ['DELIVERED', 'DISPATCHED'] } };
    if (from || to) {
      where.createdAt = {};
    ...
    for (const order of orders) {
      ...
      for (const item of order.items) {
        const gst = calculateGst(Number(item.lineTotal), item.gstRate as GstRate);
...
router.get('/busy', checkPermission('exports', 'view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { from, to } = req.query as { from?: string; to?: string };
    const where: Record<string, unknown> = { status: { in: ['DELIVERED', 'DISPATCHED'] } };
    if (from || to) {
...
router.get('/orders', checkPermission('exports', 'view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { from, to } = req.query as { from?: string; to?: string };
