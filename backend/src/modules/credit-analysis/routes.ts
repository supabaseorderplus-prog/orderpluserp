...
router.get('/', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const { data: partyVerify, error: partyErr } = await supabaseAdmin
      .from('parties')
    ...
    if ((partyVerify as unknown as { is_verified: boolean }).is_verified !== true) {
    ...
    type Invoice = { id: string; invoice_number: string; invoice_date: string; due_date: string | null; grand_total: number; amount_paid: number; amount_outstanding: number; payment_status: string; is_cancelled: boolean };
    ...
    const unpaidInvoices = nonCancelledInvoices.filter(
    ...
    const monthsDiff =
      (today.getFullYear() - firstInvoiceDate.getFullYear()) * 12 +
    ...
    const paymentToInvoiceRatio =
      avgMonthlyInvoiced > 0 ? Math.min(100, (avgMonthlyPayment / avgMonthlyInvoiced) * 100) : 0;
    ...
    let score1 = 0;
    if (paymentRate >= 95) score1 = 40;
    else if (paymentRate >= 85) score1 = 34;
    else if (paymentRate >= 70) score1 = 26;
    else if (paymentRate >= 50) score1 = 16;
    else if (paymentRate >= 30) score1 = 8;
    else score1 = Math.max(0, (paymentRate / 30) * 8);

    let score2 = 0;
    if (unpaidInvoices.length === 0) score2 = 25;
    else if (avgOutstandingDays <= 30) score2 = 25;
    else if (avgOutstandingDays <= 60) score2 = 18;
    else if (avgOutstandingDays <= 90) score2 = 12;
    else if (avgOutstandingDays <= 180) score2 = 6;
    else score2 = 0;

    let score3 = 0;
    if (paymentFrequency >= 80) score3 = 20;
    else if (paymentFrequency >= 60) score3 = 15;
    else if (paymentFrequency >= 40) score3 = 10;
    else if (paymentFrequency >= 20) score3 = 5;
    else score3 = Math.max(0, (paymentFrequency / 20) * 5);

    let score4 = 0;
    if (paymentToInvoiceRatio >= 100) score4 = 15;
    else if (paymentToInvoiceRatio >= 80) score4 = 12;
    else if (paymentToInvoiceRatio >= 60) score4 = 8;
    else if (paymentToInvoiceRatio >= 40) score4 = 4;
    else score4 = Math.max(0, (paymentToInvoiceRatio / 40) * 4);

    const rawScore = Math.round(score1 + score2 + score3 + score4);
    const creditScore = Math.round(300 + (rawScore / 100) * 600);

    let band: 'EXCELLENT' | 'GOOD' | 'FAIR' | 'POOR' | 'VERY_POOR';
    if (creditScore >= 750) band = 'EXCELLENT';
    else if (creditScore >= 650) band = 'GOOD';
    else if (creditScore >= 550) band = 'FAIR';
    else if (creditScore >= 450) band = 'POOR';
    else band = 'VERY_POOR';
